...
//                飞机模式才更新灰色线条
    ...
    private void updateLineProgress(float start, float end, boolean isLightPoints) {
        ...
        if (isLightPoints) {
            ...
            Log.i("mLightPoints",mLightPoints!=null?GsonUtil.getJsonStringFromObject(mLightPoints):"null ");
            mLightLineSemaphore.release();
        } else {
            try {
                mDarkLineSemaphore.acquire();
            ...
            Log.i("mDarkPoints",mDarkPoints!=null? GsonUtil.getJsonStringFromObject(mDarkPoints):"null");
            mDarkLineSemaphore.release();
        }
    }
    ...
    private static class Keyframes {
        ...
        float[] getRangeValue(float start, float end) {
            ...
            Log.i("getRangeValue","startIndex = "+startIndex+" endIndex = "+endIndex);
